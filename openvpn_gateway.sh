...
echo ""
echo "title           : openvpn_gateway.sh"
echo "description     : Automatic access point setup with OpenVPN for raspberry pi"
echo "author          : Wilfried Lucha (wilfried@lucha.at)"
echo "authorsite      : www.lucha.at"
echo "date            : 20160105"
echo "version         : 1.3"
echo "usage           : sudo ./openvpn_gateway.sh"
echo "disclaimer      : This script is based on ap_setup_final.sh from Jacek Tokar and Tomasz Szczerba."
echo "                  Check out the credentials below for more informations"
...
echo ""
apt-get -y install hostapd isc-dhcp-server iptables unzip                                               | tee -a ${log_file} 
...
# Improvements for DHCP failure
echo "sudo service hostapd stop"                        >> /etc/rc.local
echo "sudo service isc-dhcp-server stop"                >> /etc/rc.local
echo "sudo ifdown wlan0"                                >> /etc/rc.local
echo "sudo ifup wlan0"                                  >> /etc/rc.local
echo "sudo service hostapd restart"                     >> /etc/rc.local
...
if [ ${reboot_answer,,} = "y" ]; then
    ...
    echo "Don't forget to reboot your Raspberry Pi before using your Wifi OpenVPN Access Point!"
fi
